<?php
	...
	function getPluginParameters($pluginInstanceID) {
		global $mysqli;
		global $db_prefix;
		
		$query = "select pc.value_key, pic.value from ".$db_prefix."plugins_instance_config pic, ".$db_prefix."plugins_config pc where pic.sensor_id=$pluginInstanceID and pc.id = pic.config_id and pc.value_type!='return'";
		$result = $mysqli->query($query);
