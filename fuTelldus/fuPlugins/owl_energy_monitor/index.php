	...
	function getVirtualSensorVal($parameter, $virtualSensorID) {
		...
		if (($handle = fopen($datafilepath, "r")) !== FALSE) {
		    ...
		    while (($data = fgetcsv($handle, 1000, " ")) !== FALSE) {
		        ...
		        // parse the date, it should be between now and the last update
		        $date = str_replace('/', '-', $date);
		        $actTimeStamp = strtotime("".$date." ".$time."");
		        $offset = date('Z');
		        $actTimeStamp = $actTimeStamp + $offset;
		        //echo "date: ".$date."; last: ".$lastCheck."; current: ".$currentTime." file: ".$actTimeStamp."\n";
		        if($actTimeStamp <= $currentTime && $actTimeStamp > $lastCheck) {
		        	$consumption = $data[2];
		        	$footprint = $data[3];
